...
var examples = [ 
  ...
  ["2", "1000", "x*x", "1/(x*x)"] ];
var show = [[1, 0, 255, 0, 0], [1, 0, 0, 0, 255], [1, 0, 0, 255, 0]];


function setup() {
  console.log("Version 001: 4/25/2018")
...
function draw() {
  ...
  //text("FPS: "+round(frameRate())+" Solve Time: "+solveTime+"ms", -width/2, -height/2+10);
  //textSize(25*windowScale);
  //text("Display: "+display, -900*windowScale, -300*windowScale);
  //text("prev2: "+prev2, -900*windowScale, -250*windowScale);
  //text("Display2: "+display2, -900*windowScale, -200*windowScale);
  //text("#1: "+number1, -900*windowScale, -150*windowScale);
  //text("#2: "+number2, -900*windowScale, -100*windowScale);
  //text("function2: "+function2, -900*windowScale, -50*windowScale);
  //text("Decimal: "+decimal, -900*windowScale, 0*windowScale);
  //text("idk: "+idk, -900*windowScale, 50*windowScale);
  //text("calcdis: "+calcdis, -900*windowScale, 100*windowScale);
  stroke(0);
  strokeWeight(4);
  ...
  button("r(\u03B8)", -440, -410, 70, 50, 1, true, 0);
...
function keyPressed() {
  ...
  if (keyCode == UP_ARROW) {
    ...
    console.log("Graph Max: "+graphMax*2+"PI");
    update();
  }
  if (keyCode == DOWN_ARROW) {
    graphMax--;
    graphMax = constrain(graphMax, 1, 10);
    console.log("Graph Max: "+graphMax*2);
    update();
  }
...
function buttonfunc(  number) {
  if (decimal == false) {
    ...
    console.log(display, number, str(number), display+number, display+str(number));
...
function store(  input, inputSelected) {
  ...
  try {     // Stops graphing if input incorrect
    ...
    for (place = 0; place < i; place++) {    // Puts new function varo string form
...
function plot(inputSelected) {
  ...
  endTime = millis();
  console.log("Final R: "+prepareToSolve(2*graphMax*PI, inputSelected));
  solveTime = endTime-startTime;
  console.log("Graph Time: "+solveTime+"ms");
  updateDraw = true;
}
